...
- **设备状态持久化**：
  - `settings.yaml`：XGbe / DSA / QSFP 设置在“应用/设置”时自动写入，页面加载时回填。
  - `config.yaml`：**本振（LO）**为单一持久化值，供**初始化本振 / 混频面板 / 频谱 x 轴**三者共用；混频面板修改后自动写回，初始化即使用该持久化本振。
...
lo_mhz: 360.0              # 本振（LO）频率（MHz）：频谱 x 轴偏移 + 初始化本振（混频联动）
